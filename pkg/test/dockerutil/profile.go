...
type Pprof struct {
	...
	HeapProfile      bool
	MutexProfile     bool
	Duration         string // duration to run profiler e.g. '10s' or '1m'.
	shouldRun        bool
	cmd              *exec.Cmd
...
func MakePprofFromFlags(c *Container) *Pprof {
	...
	return &Pprof{
		BasePath:         filepath.Join(*baseDir, c.Runtime, c.Name),
...
func (p *Pprof) OnStart(c *Container) error {
	...
	go func() {
		...
		if err := p.cmd.Start(); err != nil {
			panic(fmt.Sprintf("process failed: %v", err))
		}
	}()
	return nil
}
...
func (p *Pprof) OnCleanUp(c *Container) {
	if p.cmd != nil && p.cmd.Process != nil && p.cmd.ProcessState != nil && !p.cmd.ProcessState.Exited() {
		if err := p.cmd.Process.Kill(); err != nil {
			c.logger.Logf("cleanup failed to kill runsc debug: %v", err)
		}
	}
}
