...
var (
	...
	// native is always runc.
	native = "runc"

	// The following flags are for the "pprof" profiler tool.

	// baseDir allows the user to change the directory to which profiles are
	// written. By default, profiles will appear under:
	// /tmp/profile/RUNTIME/CONTAINER_NAME/*.pprof.
	baseDir = flag.String("pprof-dir", "/tmp/profile", "base directory in: BASEDIR/RUNTIME/CONTINER_NAME/FILENAME (e.g. /tmp/profile/runtime/mycontainer/cpu.pprof)")
	...
	duration = flag.Int("pprof-duration", 10, "duration to run the profile in seconds")
