...
type Timer interface {
	...
	//
	// This cannot be done concurrent to other calls to the Timer's Stop method.
	//
	// If t.Stop returns false, then the timer has already expired and the
	// function f of Clock.AfterFunc(d, f) has been started in its own goroutine;
	// Stop does not wait for f to complete before returning. If the caller needs
