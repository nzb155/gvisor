		...
		for _, tc := range testCases {
			...
			b.Run(testName, func(b *testing.B) {
				...
				if err := container.Spawn(ctx, dockerutil.RunOpts{
					...
					"sleep", fmt.Sprintf("%d", 1000*b.N)); err != nil {
				...
				if strings.Contains(tc.test, "read") {
					ddCmd := fmt.Sprintf("dd if=/dev/random of=%s bs=1 count=0 seek=%s", outfile, tc.size)
					if out, err := container.Exec(ctx, dockerutil.ExecOpts{},
						strings.Split(ddCmd, " ")...); err != nil {
...
func makeMount(machine harness.Machine, mountType mount.Type, target string) (mount.Mount, func(), error) {
	...
	case mount.TypeBind:
		...
		if err != nil {
			defer machine.RunCommand("rm", "-rf", dir)
...
func parseFioJSON(data, op, metric string) (float64, error) {
	...
	if err := json.Unmarshal([]byte(data), &c); err != nil {
		return 0, fmt.Errorf("could not unmarshal: %v", err)
	}
	// We should find things at c["jobs"][0][op][metric].
	j, ok := c["jobs"]
	if !ok {
		return 0, fmt.Errorf("no jobs declared: %v", c)
	}
	jl, ok := j.([]interface{})
	if !ok {
		return 0, fmt.Errorf("improper format: %v", j)
	}
	jm, ok := jl[0].(map[string]interface{})
	if !ok {
		return 0, fmt.Errorf("could not find first element: %v", jl)
	}
	j, ok = jm[op]
	if !ok {
		return 0, fmt.Errorf("could not find write field: %v", jm)
	}
	...
	j, ok = jm[metric]
	if !ok {
		return 0, fmt.Errorf("could not find bandwidth field: %v", jm)
	}
	result, ok := j.(float64)
	if !ok {
		return 0, fmt.Errorf("found value not a string: %v", j)
	}
	return result, nil
}
